...
<head>
    <meta charset="UTF-8">
    <title>FAQ — RSU Allam Medica</title>
    ...
    h1, h2, h3, h4, .kontak-form-title, .bs-form-title {
    ...
    body {
        ...
        background: #ffffff;
        overflow-x: hidden;
        padding-top: 38px; /* Hero langsung nempel di bawah topbar */
        position: relative;
    }
    ...
    /* Dropdown Layanan Ramping (2 Kolom) */
    .drop-menu-layanan { 
        min-width: 420px; 
    ...
    .kontak-form-title, .bs-form-title { font-size: 19px; color: #FEFCF1; margin-bottom: 16px; line-height: 1.2; }
    ...
    /* ============================================================
       FAQ HERO & SPECIFIC
    ============================================================ */
    .faq-hero { background: linear-gradient(150deg, #1C145C 0%, #231a72 40%, #0ea5e9 100%); padding: 85px 0 72px; position: relative; overflow: hidden; }
    .faq-hero::before { content: ''; position: absolute; right: -80px; top: -80px; width: 420px; height: 420px; border-radius: 50%; background: radial-gradient(circle, rgba(255,255,255,.06), transparent 65%); pointer-events: none; }
    .faq-hero::after { content: ''; position: absolute; left: -40px; bottom: -100px; width: 300px; height: 300px; border-radius: 50%; background: radial-gradient(circle, rgba(14,165,233,.12), transparent 65%); pointer-events: none; }
    ...
    .faq-body { padding: 64px 0 80px; background: #fff; position: relative; overflow: hidden; }
    .faq-body::after{ content:''; position:absolute; right:-120px; bottom:80px; width:420px; height:420px; background:url('{{ asset("images/beranda/ornamen.png") }}') center/contain no-repeat; opacity:.05; pointer-events:none; z-index:0; }
    .faq-body::before{ content:''; position:absolute; left:-120px; top:80px; width:340px; height:340px; background:url('{{ asset("images/beranda/ornamen.png") }}') center/contain no-repeat; opacity:.03; pointer-events:none; z-index:0; }
    .faq-body > .container{ position:relative; z-index:2; }

    .faq-search-wrap { position: relative; margin-bottom: 32px; max-width: 560px; }
    .faq-search-wrap i { position: absolute; left: 18px; top: 50%; transform: translateY(-50%); color: #94a3b8; font-size: 14px; pointer-events: none; }
    .faq-search { width: 100%; padding: 14px 18px 14px 46px; border: 1.5px solid #e2e8f0; border-radius: 50px; font-family: 'Plus Jakarta Sans', sans-serif; font-size: 14px; color: #1e293b; outline: none; background: #f8faff; box-shadow: 0 2px 10px rgba(28,20,92,.05); transition: border-color .2s, box-shadow .2s; }
    .faq-search::placeholder { color: #b0bec5; }
    .faq-search:focus { border-color: #0ea5e9; box-shadow: 0 0 0 4px rgba(14,165,233,.12); background: #fff; }

    .faq-layout { display: grid; grid-template-columns: 250px 1fr; gap: 44px; align-items: start; }
    .faq-cats { position: sticky; top: 96px; }
    .faq-cats-title { font-size: 11px; font-weight: 800; color: #b7bfcc; letter-spacing: .1em; text-transform: uppercase; margin-bottom: 14px; }
    .faq-cat-btn { display: flex; align-items: center; justify-content: space-between; gap: 8px; width: 100%; text-align: left; padding: 10px 14px; border-radius: 12px; border: none; background: transparent; font-size: 13.5px; font-weight: 700; color: #64748b; cursor: pointer; margin-bottom: 3px; transition: all .2s; font-family: 'Plus Jakarta Sans', sans-serif; }
    .faq-cat-btn:hover { background: #eff6ff; color: #1C145C; }
    .faq-cat-btn.active { background: #1C145C; color: #fff; }
    .faq-cat-btn .cat-count { display: inline-flex; align-items: center; justify-content: center; min-width: 20px; height: 20px; padding: 0 5px; border-radius: 50%; background: rgba(28,20,92,.1); color: #1C145C; font-size: 10.5px; font-weight: 800; }
    .faq-cat-btn.active .cat-count { background: rgba(255,255,255,.22); color: #fff; }

    .faq-group-label { font-size: 11px; font-weight: 800; color: #b7bfcc; letter-spacing: .1em; text-transform: uppercase; margin: 26px 0 12px; }
    .faq-group-label:first-child { margin-top: 0; }

    .faq-list { display: flex; flex-direction: column; gap: 12px; }
    .faq-item { background: #fff; border-radius: 16px; overflow: hidden; border: 1.5px solid #eef1f7; box-shadow: 0 2px 10px rgba(28,20,92,.04); transition: border-color .25s, box-shadow .25s; }
    .faq-item.open { border-color: #0ea5e9; box-shadow: 0 10px 28px rgba(14,165,233,.12); }
    .faq-q { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 19px 22px; cursor: pointer; background: none; border: none; width: 100%; text-align: left; font-family: 'Plus Jakarta Sans', sans-serif; }
    .faq-q-left { display: flex; align-items: flex-start; gap: 13px; }
    .faq-q-icon { width: 36px; height: 36px; border-radius: 11px; flex-shrink: 0; background: rgba(28,20,92,.06); color: #1C145C; display: flex; align-items: center; justify-content: center; font-size: 15px; transition: background .25s, color .25s; }
    .faq-item.open .faq-q-icon { background: #1C145C; color: #fff; }
    .faq-q-text { font-size: 14.5px; font-weight: 700; color: #1e293b; line-height: 1.5; }
    .faq-item.open .faq-q-text { color: #0c4a6e; }
    .faq-q-cat { font-size: 10px; font-weight: 800; color: #0ea5e9; text-transform: uppercase; letter-spacing: .6px; margin-bottom: 3px; display:block; }
    .faq-toggle { width: 30px; height: 30px; border-radius: 50%; flex-shrink: 0; background: #f1f5f9; display: flex; align-items: center; justify-content: center; font-size: 15px; font-weight: 700; color: #94a3b8; transition: all .3s ease; }
    .faq-item.open .faq-toggle { background: #1C145C; color: #fff; transform: rotate(45deg); }

    .faq-a-wrap { max-height: 0; overflow: hidden; transition: max-height .35s ease; }
    .faq-item.open .faq-a-wrap { max-height: 500px; }
    .faq-a { padding: 0 22px 22px 71px; font-size: 13.5px; color: #64748b; line-height: 1.85; }
    ...
    .faq-cta { margin-top: 40px; border-radius: 22px; background: #f4f6fb; border: 1.5px solid #e4e9f4; padding: 40px 30px; text-align: center; position: relative; overflow: hidden; }
    .faq-cta::before { content: ''; position: absolute; right: -60px; top: -60px; width: 220px; height: 220px; border-radius: 50%; background: radial-gradient(circle, rgba(28,20,92,.06), transparent 65%); pointer-events: none; }
    .faq-cta-text h4 { font-family: 'Plus Jakarta Sans', sans-serif; font-size: 19px; font-weight: 800; color: #1C145C; margin-bottom: 6px; position: relative; z-index: 1; }
    .faq-cta-text p { font-size: 13.5px; color: #64748b; margin: 0 0 22px; position: relative; z-index: 1; }
    .faq-cta-btns { display: flex; align-items: center; justify-content: center; gap: 10px; flex-wrap: wrap; position: relative; z-index: 1; }
    .faq-cta-btn { display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; border-radius: 50px; background: #1C145C; color: #fff; font-size: 13.5px; font-weight: 700; text-decoration: none; transition: transform .2s, box-shadow .2s; box-shadow: 0 10px 24px rgba(28,20,92,.22); }
    .faq-cta-btn:hover { transform: translateY(-2px); color: #fff; box-shadow: 0 14px 30px rgba(28,20,92,.28); }
    .faq-cta-btn-ghost { display: inline-flex; align-items: center; gap: 8px; padding: 12px 24px; border-radius: 50px; background: #fff; color: #1C145C; border: 1.5px solid #dbe1ee; font-size: 13.5px; font-weight: 700; text-decoration: none; transition: border-color .2s, color .2s; }
    .faq-cta-btn-ghost:hover { border-color: #1C145C; color: #1C145C; }

    @media(max-width:991.98px) {
        .faq-layout { grid-template-columns: 1fr; gap: 22px; }
        .faq-cats { position: static; display: flex; gap: 8px; flex-wrap: wrap; }
        .faq-cats-title { display: none; }
        .faq-cat-btn { width: auto; padding: 8px 16px; border: 1.5px solid #e2e8f0; border-radius: 50px; }
        .faq-cat-btn.active { border-color: #1C145C; }
        .faq-a { padding-left: 22px; }
    }
    @media(max-width:575.98px) {
        .faq-hero { padding: 80px 0 56px; }
        .faq-search { padding: 12px 16px 12px 42px; }
        .faq-q { padding: 15px 16px; }
        .faq-a { padding: 0 16px 16px 16px; }
        .faq-q-icon { width: 30px; height: 30px; font-size: 12px; }
    ...
    @media(max-width: 575px) {
        .wa-float-btn { right: 20px; bottom: 20px; width: 60px; height: 60px; } /* Di FAQ bottom 20px aman karena tidak ada floating bar */
...
$totalFaq = collect($faqGroups)->sum(fn($g) => count($g['items']));
@endphp
...
<section class="faq-hero">
    ...
    <div class="container hero-inner">
        ...
        <div class="hero-kat">
            <i class="fa-regular fa-circle-question" style="font-size:10px;"></i>
            Pusat Bantuan
        </div>
        <h1 class="hero-title">Ada yang ingin Anda<br><em>ketahui?</em></h1>
        <div class="hero-meta">
...
<section class="faq-body">
    <div class="container">
        ...
        <div class="faq-search-wrap">
            ...
            <input type="search" class="faq-search" id="searchFaq" placeholder="Cari pertanyaan...">
        </div>

        <div class="faq-layout">

            {{-- SIDEBAR KATEGORI (sticky) --}}
            <div class="faq-cats" id="faqCatRow">
                <div class="faq-cats-title">Kategori</div>
                <button type="button" class="faq-cat-btn active" data-kat="">
            ...
            <div>
                <div class="faq-list" id="faqList">
                        ...
                        <div class="faq-item" data-kat="{{ strtolower(str_replace([' ', '&'], ['-', 'and'], $group['kategori'])) }}" data-q="{{ strtolower($item['q']) }}">
                            <button type="button" class="faq-q" data-target="faqA{{ $key }}">
                                <div class="faq-q-left">
                                    ...
                                    <div>
                                        <span class="faq-q-cat">{{ $group['kategori'] }}</span>
                                        <span class="faq-q-text">{{ $item['q'] }}</span>
                                    </div>
                                </div>
                                <span class="faq-toggle">+</span>
                            </button>
                            <div class="faq-a-wrap" id="faqA{{ $key }}">
                    ...
                    <div class="empty-state">
                        <div class="es-icon"><i class="fa-regular fa-circle-question"></i></div>
                        <div class="es-title">Belum Ada FAQ</div>
                        <div class="es-sub">Pertanyaan akan segera ditambahkan.</div>
                    </div>
                    @endforelse

                    <div class="empty-state" id="faqEmptyState" style="display:none;">
                        <div class="es-icon"><i class="fa-solid fa-magnifying-glass"></i></div>
                        <div class="es-title">Tidak Ditemukan</div>
                        <div class="es-sub">Coba kata kunci atau kategori lain.</div>
                    </div>
                </div>
                ...
                <div class="faq-cta">
                    ...
                    <div class="faq-cta-btns">
                        <a href="https://wa.me/6285292224886" target="_blank" class="faq-cta-btn">
                            <i class="fa-brands fa-whatsapp"></i> Chat WhatsApp
                        </a>
                        <a href="mailto:allam.medica@yahoo.co.id" class="faq-cta-btn-ghost">
...
<!-- ============================================================
     JAVASCRIPT: NAVBAR, FAQ LOGIC, WA MODAL
============================================================ -->
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ─── FAQ Kategori Filter (sidebar sticky) & Search ─── */
    const catRow = document.getElementById('faqCatRow');
    const faqSearch = document.getElementById('searchFaq');
    const faqGroupsEl = document.querySelectorAll('#faqList .faq-group');
    ...
    function applyFaqFilter() {
        ...
        faqGroupsEl.forEach(group => {
            const matchGroupKat = !activeKat || group.dataset.kat === activeKat;
            let groupHasVisible = false;

            group.querySelectorAll('.faq-item').forEach(item => {
                const matchQ = !q || item.dataset.q.includes(q);
                const show = matchGroupKat && matchQ;
            ...
            if (label) label.style.display = (groupHasVisible && !activeKat) ? '' : (groupHasVisible ? 'none' : 'none');
        });
    ...
    if (catRow) {
        catRow.querySelectorAll('.faq-cat-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                catRow.querySelectorAll('.faq-cat-btn').forEach(b => b.classList.remove('active'));

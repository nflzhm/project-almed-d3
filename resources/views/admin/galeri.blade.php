...
.am-modal .modal-body { padding:24px; }
...
<div class="page-header">
    <div class="page-header-left">
        ...
        <div class="ph-sub">Kelola foto fasilitas, kegiatan, dokter, dan event RS</div>
    </div>
    <button class="btn-primary-am" data-bs-toggle="modal" data-bs-target="#modalTambah">
        <i class="fa-solid fa-image"></i> Tambah Foto
    </button>
</div>
...
<div class="gal-stats">
    <div class="gal-stat">
        ...
        <div><div class="gal-stat-val">{{ $totalGaleri }}</div><div class="gal-stat-lbl">Total Foto</div></div>
    </div>
    <div class="gal-stat">
    ...
    <div class="gal-card" data-kategori="{{ $item->kategori }}">
        <div class="gal-photo-wrap">
            ...
            <div class="gal-actions">
                <button class="gal-action-btn gab-edit"
                    onclick="openEditModal('{{ $item->id }}', `{{ addslashes($item->judul) }}`, '{{ $item->kategori }}', `{{ addslashes($item->deskripsi) }}`, '{{ asset('uploads/galeri/'.$item->gambar) }}')">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa-solid fa-image"></i> Tambah Foto Galeri</h5>
            ...
            <form action="{{ route('admin.galeri.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="mfg">
                        <div class="mfg-label">Judul Foto <span style="color:#ef4444;">*</span></div>
                    ...
                    <div class="mfg">
                        <div class="mfg-label">Foto <span style="color:#ef4444;">*</span></div>
                        <input type="file" name="gambar" class="mfg-input" accept="image/jpeg,image/png,image/webp" required>
                        <div style="font-size:11px;color:var(--text-muted);margin-top:4px;">JPG, PNG, WebP — Maks. 5 MB</div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header" style="background:linear-gradient(135deg,#1e3a5f 0%,#0ea5e9 100%);">
                <h5 class="modal-title"><i class="fa-solid fa-pen-to-square"></i> Edit Foto Galeri</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="" method="POST" enctype="multipart/form-data" id="formEdit">
                @csrf @method('PUT')
                <div class="modal-body">
                    <img id="editPreview" src="" style="width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:12px;margin-bottom:16px;border:1.5px solid var(--border-color);">
                    <div class="mfg">
                        <div class="mfg-label">Judul Foto <span style="color:#ef4444;">*</span></div>
                    ...
                    <div class="mfg">
                        <div class="mfg-label">Ganti Foto <span style="color:var(--text-muted);">(kosongkan jika tidak diganti)</span></div>
                        <input type="file" name="gambar" class="mfg-input" accept="image/jpeg,image/png,image/webp">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-cancel" data-bs-dismiss="modal">Batal</button>
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px;">
        <div class="modal-content">
            <div class="modal-header" style="background:linear-gradient(135deg,#7f1d1d 0%,#ef4444 100%);">
                <h5 class="modal-title"><i class="fa-solid fa-triangle-exclamation"></i> Hapus Foto</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="" method="POST" id="formHapus">
                @csrf @method('DELETE')
                <div class="modal-body text-center">
                    <p>Hapus foto <strong id="delTarget">—</strong> secara permanen?</p>
                </div>
...
@push('scripts')
<script>
function openEditModal(id, judul, kategori, deskripsi, foto) {
    document.getElementById('editJudul').value = judul;
    document.getElementById('editKategori').value = kategori;
    document.getElementById('editDeskripsi').value = deskripsi;
    document.getElementById('editPreview').src = foto;
...
function filterGaleri() {
    ...
    document.querySelectorAll('#galGrid .gal-card').forEach(function (card) {

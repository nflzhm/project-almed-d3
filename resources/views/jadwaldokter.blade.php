...
<head>
    ...
    body {
        ...
        background: #ffffff;
        overflow-x: hidden;
        padding-top: 38px; /* REVISI: Nempel tepat di bawah Topbar */
        position: relative;
        margin: 0;
    }
    ...
    .hero-jadwal .hero-ornament { position: absolute; right: -80px; top: -80px; width: 460px; height: 460px; opacity: .07; background-image: url('{{ asset("images/beranda/ornamen.png") }}'); background-size: contain; background-repeat: no-repeat; background-position: center; pointer-events: none; z-index: 0; }
    .hero-jadwal .hero-content { position: relative; z-index: 2; }
    .hero-jadwal .hero-eyebrow { font-size: 11px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: rgba(255,255,255,0.65); margin-bottom: 12px; display: block; }
    .hero-jadwal h1 { font-size: clamp(26px, 4vw, 44px); font-weight: 900; color: #fff; line-height: 1.15; margin-bottom: 14px; }
    .hero-jadwal p { font-size: 15px; color: rgba(255,255,255,0.82); line-height: 1.75; max-width: 520px; margin: 0; }
    .hero-jadwal .hero-wave { position: absolute; bottom: -1px; left: 0; width: 100%; line-height: 0; z-index: 1; }
    ...
    .schedule-section {
        ...
        overflow: hidden;
        padding: 55px 0 70px;
        background: linear-gradient(to bottom, #ffffff 0%, #fdfcf6 5%, #f7f5e8 18%, #f0ede3 35%, #ede9d9 50%, #f0ede3 65%, #f7f5e8 82%, #fdfcf6 95%, #ffffff 100%);
    }
    .schedule-section .sched-ornament-tr { position: absolute; right: -100px; top: -60px; width: 420px; height: 520px; opacity: .045; background-image: url('{{ asset("images/beranda/ornamen.png") }}'); background-size: contain; background-repeat: no-repeat; background-position: center; pointer-events: none; z-index: 0; }
    .schedule-section .sched-ornament-bl { position: absolute; left: -120px; bottom: -100px; width: 480px; height: 620px; opacity: .04; background-image: url('{{ asset("images/beranda/ornamen.png") }}'); background-size: contain; background-repeat: no-repeat; background-position: center; pointer-events: none; z-index: 0; }
    .schedule-section .sched-ornament-mid { position: absolute; right: 10%; bottom: 20%; width: 300px; height: 300px; opacity: .025; background-image: url('{{ asset("images/beranda/ornamen.png") }}'); background-size: contain; background-repeat: no-repeat; background-position: center; pointer-events: none; z-index: 0; }
    .schedule-section .sched-ornament-tl { position: absolute; left: -110px; top: 30%; width: 360px; height: 360px; opacity: .03; background-image: url('{{ asset("images/beranda/ornamen.png") }}'); background-size: contain; background-repeat: no-repeat; background-position: center; pointer-events: none; z-index: 0; }
    .schedule-section .container { position: relative; z-index: 1; }

    .schedule-section-title { margin-bottom: 28px; }
    .schedule-section-title .eyebrow { font-size: 11px; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: #8a84b8; display: block; margin-bottom: 4px; }
    .schedule-section-title h2 { font-size: 26px; font-weight: 900; color: #1C145C; margin: 0; }

    .schedule-filter { background: rgba(255,255,255,0.75); backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px); border-radius: 20px; padding: 22px 24px; box-shadow: 0 8px 28px rgba(28,20,92,.08); margin-bottom: 30px; border: 1px solid rgba(255,255,255,0.85); }
    .search-wrap { display: flex; gap: 10px; margin-bottom: 16px; }
    .search-input { flex: 1; height: 46px; border: 1.5px solid rgba(28,20,92,0.15); border-radius: 12px; padding: 0 16px; font-size: 14px; background: #fff; color: #1e293b; outline: none; transition: border-color .2s; }
    .search-input:focus { border-color: #1C145C; box-shadow: 0 0 0 3px rgba(28,20,92,.08); }
    .search-btn { background: #1C145C; color: #fff; border: none; border-radius: 12px; padding: 0 24px; font-size: 14px; font-weight: 600; height: 46px; white-space: nowrap; cursor: pointer; transition: .2s; display: inline-flex; align-items: center; gap: 7px; }
    .search-btn:hover { background: #2a1e8a; transform: translateY(-1px); }

    .filter-title { font-size: 12px; font-weight: 700; margin-bottom: 10px; color: #1C145C; text-transform: uppercase; letter-spacing: 0.08em; }
    .day-list { display: flex; gap: 8px; flex-wrap: wrap; }
    .day-list a { text-decoration: none; background: rgba(28,20,92,.06); color: #5a5480; padding: 6px 18px; border-radius: 30px; font-size: 12.5px; font-weight: 600; border: 1.5px solid rgba(28,20,92,.1); transition: .2s; }
    .day-list a:hover { background: rgba(28,20,92,.12); color: #1C145C; }
    .day-list a.active { background: #1C145C; color: #fff; border-color: #1C145C; box-shadow: 0 4px 12px rgba(28,20,92,.25); }

    .schedule-box { background: rgba(255,255,255,0.80); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border-radius: 20px; padding: 20px; box-shadow: 0 6px 24px rgba(28,20,92,.07); border: 1px solid rgba(255,255,255,0.9); margin-bottom: 14px; transition: box-shadow .25s, transform .25s; }
    .schedule-box:hover { box-shadow: 0 12px 36px rgba(28,20,92,.12); transform: translateY(-2px); }

    .schedule-grid { display: grid; grid-template-columns: 260px 1fr; gap: 20px; }
    .doctor-card { background: linear-gradient(135deg, #1C145C, #2d2391); border-radius: 16px; padding: 24px 20px; color: #fff; text-align: center; position: relative; overflow: hidden; }
    .doctor-card::after { content: ''; position: absolute; right: -30px; bottom: -30px; width: 130px; height: 130px; background: rgba(255,255,255,.05); border-radius: 50%; pointer-events: none; }
    .doctor-card img { width: 90px; height: 90px; border-radius: 50%; object-fit: cover; margin-bottom: 14px; border: 3px solid rgba(255,255,255,.25); display: block; margin-left: auto; margin-right: auto; }
    .doc-info { display: flex; flex-direction: column; align-items: center; }
    .doctor-name { font-size: 14px; font-weight: 700; line-height: 1.5; font-family: 'GothamBlack', sans-serif; }
    .doctor-specialist { font-size: 12px; font-weight: 500; color: rgba(255,255,255,.8); margin-top: 6px; line-height: 1.5; background: rgba(255,255,255,.1); border-radius: 20px; padding: 4px 12px; display: inline-block; margin-top: 10px; }

    .practice-title { font-size: 12px; font-weight: 800; color: #1C145C; margin-bottom: 14px; letter-spacing: .1em; text-transform: uppercase; }
    .day-row { display: grid; grid-template-columns: repeat(7, 1fr); gap: 10px; }

    .day-col { background: #fff; border-radius: 12px; overflow: hidden; border: 1.5px solid #ece9f1; transition: box-shadow .2s; }
    .day-col:not(.libur):hover { box-shadow: 0 6px 18px rgba(28,20,92,.1); }
    .day-head { background: #1C145C; color: #fff; font-size: 12px; font-weight: 700; text-align: center; padding: 9px 6px; }
    .day-body { padding: 12px 8px; text-align: center; }
    .clinic { font-size: 11.5px; font-weight: 600; color: #334155; margin-bottom: 7px; min-height: 30px; line-height: 1.4; }
    .time { font-size: 13px; font-weight: 800; color: #1C145C; margin-bottom: 7px; }
    .note { font-size: 11px; color: #94a3b8; line-height: 1.5; }

    .libur { opacity: 0.55; }
    .libur .day-head { background: #8b8aaa; }
    .libur .clinic, .libur .time, .libur .note { color: #b5b5b5; }

    .no-result { text-align: center; padding: 60px 20px; color: #8a84b8; display: none; }
    .no-result i { font-size: 48px; margin-bottom: 14px; display: block; opacity: .5; }
    ...
    @media(max-width:991px) {
        ...
        .doctor-card { display: flex; align-items: center; gap: 12px; text-align: left; padding: 12px 14px; border-radius: 14px; cursor: pointer; position: relative; }
        .doctor-card::after { display: none; }
        .doctor-card img { width: 60px; height: 60px; margin: 0; flex-shrink: 0; border-width: 2px; }
        .doctor-card .doc-info { flex: 1; min-width: 0; }
        .doctor-name { font-size: clamp(11px, 3.2vw, 14px); margin: 0; line-height: 1.4; white-space: normal; word-break: break-word; }
        .doctor-specialist { font-size: clamp(10px, 2.6vw, 12px); margin-top: 5px; padding: 3px 9px; display: inline-block; }
        .doctor-card .toggle-arrow { margin-left: auto; flex-shrink: 0; font-size: 12px; color: rgba(255,255,255,.7); transition: transform .3s; }
        .schedule-box.active .doctor-card .toggle-arrow { transform: rotate(180deg); }
        .practice-area { display: none; margin-top: 12px; }
        .schedule-box.active .practice-area { display: block; }
        .day-row { grid-template-columns: repeat(2, 1fr); gap: 8px; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (waCancelBtn) {
        waCancelBtn.addEventListener('click', contentHandler = closeWaModal);
    }

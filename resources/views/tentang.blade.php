...
.btn-kontak {
    padding:10px 22px;border-radius:50px;background:#1C145C;color:#fff!important;
    text-decoration:none!important;font-size:14px;font-weight:600;display:inline-flex;
    align-items:center;gap:6px;border:none;box-shadow:0 8px 20px rgba(28,20,92,.25);
...
.kontak-mega::before {
    ...
    background:linear-gradient(90deg,transparent,rgba(28,20,92,.2) 50%,transparent);border-radius:2px;
}
...
.bs-sheet {
    position:fixed;left:0;right:0;bottom:0;z-index:10000002;background:#fff;
    border-radius:24px 24px 0 0;box-shadow:0 -8px 40px rgba(15,23,42,.18);
    transform:translateY(100%);transition:transform .35s cubic-bezier(.4,0,.2,1);
    height:92dvh;max-height:92dvh;display:flex;flex-direction:column;overflow:hidden;
}
...
.bs-body {
    ...
    overscroll-behavior:contain;padding:18px;
...
.bs-form-card .ck-field textarea {
    ...
    border:1px solid rgba(254,252,241,.18);color:#FEFCF1;width:100%;box-sizing:border-box;
...
.hero-section{
    position: relative;
    margin-top: calc(-38px - 94px);
    padding-top: calc(38px + 94px);
    background:
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function bsIsOpen() { return bsSheet.classList.contains('open'); }
    function openBs()   { bsSheet.classList.add('open');bsOverlay.classList.add('show');document.body.style.overflow='hidden'; }
    ...
    function closeDrawer() { burger.classList.remove('open');drawer.classList.remove('open');overlay.classList.remove('show');if(!bsIsOpen())document.body.style.overflow=''; }
    ...
    /* ─── Bottom Sheet Mobile ─── */
    btnMobile.addEventListener('click', () => { openBs(); closeDrawer(); });
